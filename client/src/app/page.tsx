  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
      <main className="max-w-2xl mx-auto">
        ...
        >
          <div className={`sticky top-0 z-10 ${isDarkMode ? 'bg-gray-900/80 border-gray-800' : 'bg-white/80 border-gray-200'} backdrop-blur-md`}>
            <div className={`px-4 py-3 border-b ${isDarkMode ? 'border-gray-800' : 'border-gray-200'}`}>
              <h1 className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Accueil</h1>
            </div>
            ...
            {searchResults ? (
              <div className="p-4">
                <SearchResults results={searchResults} onTweetUpdate={handleTweetUpdate} onClear={clearSearchResults} query={searchQuery} />
              </div>
            ) : (
              ...
              {tweets.length > 0 ? (
                tweets.map(tweet => (
                  <motion.div
                    ...
                    transition={{ duration: 0.3 }}
                  >
                    <TweetCard tweet={tweet} onTweetUpdate={handleTweetUpdate} />
                  </motion.div>
                ))
              ) : (
          ...
          {!searchResults && recommendedTweets.length > 0 && (
            <div className={`mt-4 p-4 rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-purple-50'}`}>
